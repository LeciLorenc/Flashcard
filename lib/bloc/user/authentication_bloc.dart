...
class AuthenticationBloc extends Bloc<AuthenticationEvent, AuthenticationState> {
  ...
  void _signOutOnStart() async {
    await FirebaseAuth.instance.signOut(); // Sign out on app start
    FirebaseAuth.instance.userChanges().listen((User? user) {
      add(_ReloadEvent(user: user));
    });
  }

  void _onReloadEvent(_ReloadEvent event, Emitter<AuthenticationState> emit) {
    if (event.user == null) {
